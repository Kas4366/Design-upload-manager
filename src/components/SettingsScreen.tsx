...
import { Folder, Save, Settings as SettingsIcon, Check, AlertCircle, Upload, X, FileSpreadsheet, RefreshCw, Download, FolderTree } from 'lucide-react';
...
export function SettingsScreen({ onClose }: SettingsScreenProps) {
  ...
  const loadSettings = async () => {
    try {
      ...
      setError(`Database connection error: ${errorMessage}. Please check your internet connection and Supabase configuration.`);
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const handleSaveMapping = async () => {
    try {
      ...
      console.log('Saving column mappings...', mappingValues);
      ...
      if (result.success) {
        ...
        const errorMsg = result.error || 'Failed to save column mapping';
        console.error('Save failed:', errorMsg);
        setError(`Failed to save column mapping: ${errorMsg}`);
      }
    } catch (err) {
      console.error('Unexpected error in handleSaveMapping:', err);
      const errorMessage = err instanceof Error ? err.message : 'Unknown error';
      setError(`Database connection error: ${errorMessage}. Please check your internet connection and Supabase configuration.`);
    }
  };
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
        ...
        <div className="p-6 space-y-6 overflow-y-auto flex-1">
          <div>
            ...
            <p className="text-sm text-gray-600 mb-3">
              Main folder where design files will be saved
            </p>
            ...
            <div className="flex items-center justify-between mb-4">
              <div>
                ...
                <p className="text-sm text-gray-600 mt-1">
                  Manage the folders where orders will be organized
                </p>
              </div>
              <button
            ...
            <div className="p-3 bg-gray-50 border border-gray-200 rounded-lg">
              <div className="flex items-center gap-2">
                ...
                <span className="text-sm text-gray-700">
                  {folderTypesCount === 0
                    ? 'No folder types configured'
                    : `${folderTypesCount} active folder type${folderTypesCount !== 1 ? 's' : ''} configured`}
                </span>
              </div>
            </div>
          </div>
      ...
      {showFolderTypesManager && (
        <FolderTypesManager
          onClose={() => setShowFolderTypesManager(false)}
          onUpdate={loadFolderTypesCount}
        />
      )}
    </div>
  );
